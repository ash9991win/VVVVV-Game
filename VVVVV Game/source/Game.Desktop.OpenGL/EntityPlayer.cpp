...
EntityPlayer::EntityPlayer() : mSprite(), mHeight(), mWidth(), mTextureName(), mCanFlip(false), mSpriteRenderer(ResourceManager::GetShader("SpriteShader")), mCurrentSpeed(0), mCurrentFallSpeed(0), extraPush(10), mBody(nullptr), mMovable(nullptr), mFlip(nullptr)
{
...
{
	mTextureName = name;
	mSprite.SetTexture(ResourceManager::GetTexture(mTextureName));
}
				...
				if (tileName != "Barrier")
				{
					...
					else
					{
						if (mFallSpeed / abs(mFallSpeed) > 0)
						{
							...
							//((mSprite.GetPosition().y < tile->GetSprite().GetPosition().y) || (mSprite.GetPosition().y > tile->GetSprite().GetPosition().y))&& 
							if (  ((mSprite.GetPosition().y + mSprite.GetSize().y) >(tile->GetSprite().GetPosition().y + tile->GetSprite().GetSize().y)))
							{
								//check left
						...
						{
							//((mSprite.GetPosition().y + mSprite.GetSize().y > tile->GetSprite().GetPosition().y + tile->GetSprite().GetSize().y) || (mSprite.GetPosition().y + mSprite.GetSize().y < tile->GetSprite().GetPosition().y + tile->GetSprite().GetSize().y)) && 
							if ( ((mSprite.GetPosition().y) < (tile->GetSprite().GetPosition().y)))
							{
								//check left
								if (mSprite.GetPosition().x < (tile->GetSprite().GetPosition().x + tile->GetSprite().GetSize().x) && (mSprite.GetPosition().x > tile->GetSprite().GetPosition().x))
								{
