...
LevelParser::LevelParser(Game& game, Camera& camera) : mSymbolTable(MAP_SIZE), mGame(&game), mCamera(&camera), mLevelList(), mLevelIndex(0)
{
	PopulateSymbolTable();
	PopulateLevelList();
}
...
void LevelParser::PopulateSymbolTable()
{
	...
	master.ParseFromFile(LevelParser::SYMBOLS_PATH);
...
EntityTile& LevelParser::CreateTile(Entity* entity, float x, float y)
{
	...
	tile.GetSprite().SetGame(*mGame);
	tile.GetSprite().SetCamera(mCamera);



	if (tile.Name() == "TitleScreen")
	{
		width = 700;
		height = 400;
		tile.CreateSprite(textureName, u, v, width, height);
		tile.GetSprite().SetPosition(glm::vec2(50, 360));
	}
	else
	{
		tile.CreateSprite(textureName, u, v, width, height);
	...
	tile.GetSprite().SetVisbility(visible != 0);
...
EntityPlayer& LevelParser::CreatePlayer(Entity* entity, float x, float y)
{
	...
	player.GetSprite().SetGame(*mGame);
	player.GetSprite().SetCamera(mCamera);
	...
	player.GetSprite().SetVisbility(true);
...
EntityMovable & LevelParser::CreateMovable(Entity * entity, float x, float y)
{
	...
	movable.GetSprite().SetGame(*mGame);
	movable.GetSprite().SetCamera(mCamera);
