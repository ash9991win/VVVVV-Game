	...
	for (std::uint32_t i = 0; i < entities.Size(); ++i)
	{
				...
				if (tileName != "Barrier")
				{
					...
					else
					{
						...
						if (mFallSpeed / abs(mFallSpeed) > 0)
						{
							...
							if (((mSprite.GetPosition().y + mSprite.GetSize().y - 30.0f) >(tile->GetSprite().GetPosition().y + tile->GetSprite().GetSize().y)))
							{
								//check left
								if (mSprite.GetPosition().x < (tile->GetSprite().GetPosition().x + tile->GetSprite().GetSize().x) && (mSprite.GetPosition().x > tile->GetSprite().GetPosition().x))
								{
						...
						else
						{
							...
							if (((mSprite.GetPosition().y-30.0f) < (tile->GetSprite().GetPosition().y)))
							{
								//check left
								if (mSprite.GetPosition().x < (tile->GetSprite().GetPosition().x + tile->GetSprite().GetSize().x) && (mSprite.GetPosition().x > tile->GetSprite().GetPosition().x))
								{
		...
		else if (movable)
		{
			if (mSprite.IsColliding(movable->GetSprite()))
			{
				...
				if (mFallSpeed / abs(mFallSpeed) < 0)
				{
					pos.y = mMovable->GetSprite().GetPosition().y + (mSprite.GetSize().y );//(mMovable->GetSprite().GetSize().y/2.0f);
				}
				else
				{
					pos.y = mMovable->GetSprite().GetPosition().y - (mSprite.GetSize().y/2.0f);
				}
